...
RUNTIME_PATH="$DNVM_USER_HOME/runtimes/$_DNVM_RUNTIME_PACKAGE_NAME-mono.$_TEST_VERSION"

[ -f "$RUNTIME_PATH/bin/$_DNVM_RUNTIME_EXEC_NAME" ] || die "$_DNVM_COMMAND_NAME did not include '$_DNVM_RUNTIME_EXEC_NAME' command!"
...
[ ! -f "$DNVM_USER_HOME/alias/default.alias" ] || die "default alias was created despite not setting --persistant"
